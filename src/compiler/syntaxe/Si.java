...
public class Si extends MainInter {
		...
		if(this.inDebutSection())
		{
			...
			if(Global.tokens.get(start+1).getId() == Tokens.OPENBRACKET)
			{
				...
				try {
					...
					{
						//System.out.println("Si.Si()  "+exp);
						ErrorHandler.lunch(currentToken, ErrorHandler.ConditionSyntaxeNotValide);
					}
					else {
						if(Global.tokens.get(idLastOpenB).getId() != Tokens.ALORS)
						{
							ErrorHandler.Expected(Global.tokens.get(idLastOpenB-1), false, TokensWords.ALORS);
						}
					}
				}
				catch(Exception ex)
